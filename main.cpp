...
struct Fraction {
    ...
    void normalize() {
        ...
        if (den == 0) {
            // В реальном проекте лучше бросать исключение,
            // но здесь делаем простую замену на 1, чтобы не падать.
            den = 1;
        }
...
Fraction divf(const Fraction& a, const Fraction& b) {
    // (a/b) / (c/d) = (a/b) * (d/c)
    // Деление на ноль запрещено (если c == 0)
    if (isZero(b)) {
        // Вернем 0/1, а сообщение покажем в main (чтобы студенту было понятно)
        return Fraction(0, 1);
    }
